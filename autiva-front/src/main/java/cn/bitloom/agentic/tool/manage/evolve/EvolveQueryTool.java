 ...
 */
@Slf4j
public class EvolveQueryTool extends AbstractTool<EvolveQueryTool.Input> {
    ...
    private EvolveQueryTool(EvolutionEngine evolutionEngine) {
        super("evolve_query", DESCRIPTION, Input.class);
    ...
    public ToolResult execute(Input input, ToolContext context) {
        ...
        return switch (query.toLowerCase()) {
            ...
            case "recommend" -> {
                String result = evolutionEngine.recommendGenes(Collections.emptyList());
                yield ToolResult.builder().status(ToolResult.Status.SUCCESS)
                        .message("推荐基因")
                        .rawOutput(result)
                        .build();
