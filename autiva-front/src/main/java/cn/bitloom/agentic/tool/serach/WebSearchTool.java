...
public class WebSearchTool {
	...
	private WebSearchTool(SearchProvider searchProvider, int resultCount) {
	...
	public ToolResult webSearch(
		...
		try {
			...
			List<SearchResult> allResults = this.searchProvider.search(query, this.resultCount);

			if (allResults.isEmpty()) {
				logger.warn("搜索API对查询返回空响应: {}", query);
				return ToolResult.success("未找到搜索结果", Map.of("query", query, "count", 0));
			}
			...
			return ToolResult.builder().status(ToolResult.Status.SUCCESS).message(filteredResults.size() + " 条搜索结果").data(Map.of("query", query, "count", filteredResults.size())).rawOutput(JsonParser.toJson(filteredResults)).build();

		}
		catch (Exception e) {
			logger.error("执行搜索请求时出错，查询: {}", query, e);
	...
	public static class Builder {
		...
		public WebSearchTool build() {
			return new WebSearchTool(this.searchProvider, this.resultCount);
		}
