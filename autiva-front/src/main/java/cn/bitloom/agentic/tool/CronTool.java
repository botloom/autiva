...
public class CronTool implements cn.bitloom.agentic.tool.ITool {
    ...
    public ToolResult create(
        ...
        try {
            ...
            return ToolResult.success("定时任务创建成功: " + name);

        } catch (Exception e) {
            log.error("[ToolCall] cron_create - 创建失败: name={}", name, e);
            return ToolResult.failure("创建定时任务失败: " + e.getMessage());
        }
    }

    @Tool(name = "cron_list", description = "列出所有定时任务")
    public ToolResult list(ToolContext toolContext) {
    ...
    public ToolResult delete(@ToolParam(description = "任务名称") String name, ToolContext toolContext) {
        ...
        try {
            ...
            return ToolResult.success("定时任务已删除: " + name);

        } catch (Exception e) {
            log.error("[ToolCall] cron_delete - 删除失败: name={}", name, e);
            return ToolResult.failure("删除定时任务失败: " + e.getMessage());
        }
    }

    @Tool(name = "cron_trigger", description = "手动触发定时任务")
    public ToolResult trigger(@ToolParam(description = "任务名称") String name, ToolContext toolContext) {
        ...
        try {
            ...
            return ToolResult.success("定时任务已手动触发: " + name);

        } catch (Exception e) {
            log.error("[ToolCall] cron_trigger - 触发失败: name={}", name, e);
            return ToolResult.failure("触发定时任务失败: " + e.getMessage());
        }
    }
}
