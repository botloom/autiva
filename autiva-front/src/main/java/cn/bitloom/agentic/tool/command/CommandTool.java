...
/**
 * 命令执行工具，从 CommandTools 的 Command 方法拆出。
 * <p>
 * 支持前台执行、智能后台化（yield_ms）和立即后台模式。
 * </p>
 */
public class CommandTool extends AbstractTool<CommandTool.Input> {
    ...
    private static final String DESCRIPTION = """
            ...
            工作机制：
            - 持久化会话：cwd 和环境由 Shell 自身维护，无需外部持久化
            - 跨平台：Windows 使用 cmd.exe，Unix 使用 Bash
    ...
    private ToolResult executeWithYield(String command, String description, Long timeout, String workdir, Map<String, String> env, long yieldMs) {
        ...
        String id;
        try {
            id = processManager.register(result.backgroundHandle(), command, description);
        } catch (Exception e) {
            // 注册失败时销毁句柄避免泄漏
            result.backgroundHandle().destroy();
    ...
    private ToolResult startImmediateBackground(String command, String description, String workdir, Map<String, String> env) {
        try {
            PtyTerminal.PtyHandle handle = executor.startBackground(command, workdir, env);
            String id = processManager.register(handle, command, description);
