...
import cn.bitloom.agentic.workflow.config.WorkflowConfig;
import cn.bitloom.agentic.workflow.graph.Graph;
import cn.bitloom.agentic.workflow.graph.VertexNode;
import cn.bitloom.agentic.workflow.graph.VertexParam;
import cn.bitloom.exception.WorkFlowException;
import cn.bitloom.util.SpringContextUtil;
    ...
    private Flux<WorkFlowEvent> exec(Graph graph, VertexNode vertex, Set<String> executedNodeSet) {
        return Flux.defer(() -> {
            ...
            currExecFlux = Flux.concat(
                            ...
                            Mono.defer(() -> SpringContextUtil.getBean(vertex.getType(), AbstractNode.class).run(param, workFlowContext))
                    )
                    .retryWhen(
            ...
            List<Flux<WorkFlowEvent>> nextExecFluxList = graph.getOutArc(vertex.getId()).stream()
                    ...
                    .toList();
            Flux<WorkFlowEvent> nextExecFlux = nextExecFluxList.isEmpty() ? Flux.empty() : Flux.merge(nextExecFluxList);
