...
## 概述
本包实现了基于 Reactor 的统一事件系统。EventBus 提供全局双通道架构（inBox/outBox），SessionEventBus 提供会话级双通道架构。EventBus 为全局静态工具类，SessionEventBus 为每个 Session 实例持有的事件总线。inBox 通道通过 MessageChannel 枚举实现逻辑路由，将不同来源的消息（用户、系统、记忆、日记、进化）分发到对应的处理流程。outBox 通道仅转发 USER 通道的响应消息给用户。
...
### SessionEventBus
会话级事件总线（实例对象，每个 Session 持有一个），提供与 EventBus 相同的双通道架构，但作用域限定在单个会话内。

**构造方法：**
- `SessionEventBus(String sessionId)`: 创建指定会话的事件总线

**核心方法（实例方法）：**
- `inBoxPublish(message)`: 发布事件到 Inbox（智能体接收消息），eventType 默认 MESSAGE，messageChannel 默认 USER
- `inBoxPublish(message, eventType)`: 发布带事件类型的消息到 Inbox，messageChannel 通过 `MessageChannel.fromEventType(eventType)` 自动推导
- `inBoxPublish(message, eventType, messageChannel)`: 发布带事件类型和显式通道的消息到 Inbox
- `inBoxSubscribe()`: 订阅 Inbox 事件流
- `outBoxPublish(message)`: 发布消息到 Outbox（返回给用户），使用 tryEmitNext
- `outBoxSubscribe()`: 订阅 Outbox 消息流
- `markBusy()`: 标记会话为忙碌状态
- `clearBusy()`: 清除会话忙碌状态
- `isBusy()`: 检查会话是否忙碌
- `stop()`: 设置停止标记，用于中止当前流式生成
- `isStop()`: 检查会话是否已被停止
- `clearStopFlag()`: 清除会话的停止标记

**与 EventBus 的区别：**
- EventBus 是全局静态工具类，所有方法为 static，通过 sessionId 参数路由
- SessionEventBus 是实例对象，绑定到特定 Session，方法无需 sessionId 参数
- SessionEventBus 的 inBoxPublish/outBoxPublish 不需要传入 sessionId，因为构造时已绑定
- SessionManager 通过 `publishMessage()` 方法封装了对 SessionEventBus 的调用，推荐使用 SessionManager 的路由方法而非直接访问 SessionEventBus

**迁移路径：**
- `EventBus.inBoxPublish(sessionId, message)` → `sessionManager.publishMessage(sessionId, message)` 或 `session.getEventBus().inBoxPublish(message)`
- `EventBus.outBoxPublish(sessionId, message)` → `session.getEventBus().outBoxPublish(message)`
- `EventBus.markBusy(sessionId)` → `session.getEventBus().markBusy()`
- `EventBus.clearBusy(sessionId)` → `session.getEventBus().clearBusy()`
- `EventBus.isBusy(sessionId)` → `sessionManager.isSessionBusy(sessionId)` 或 `session.getEventBus().isBusy()`
- `EventBus.isStop(sessionId)` → `sessionManager.isSessionStop(sessionId)` 或 `session.getEventBus().isStop()`
- `EventBus.stop(sessionId)` → `sessionManager.stopSession(sessionId)` 或 `session.getEventBus().stop()`
- `EventBus.clearStopFlag(sessionId)` → `sessionManager.clearSessionStopFlag(sessionId)` 或 `session.getEventBus().clearStopFlag()`
- `EventBus.outBoxSubscribe()` → `session.getEventBus().outBoxSubscribe()`（通过 SessionManager 获取 Session 后订阅）
...
```java
sessionManager.publishMessage(sessionId, new UserMessage("用户请求内容"));
```

### 发布带事件类型的消息到 Inbox（自动推导 MessageChannel）
```java
sessionManager.publishMessage(sessionId, new UserMessage("整理记忆"), EventType.MEMORY_CONSOLIDATE);
```

### 发布带显式 MessageChannel 的消息到 Inbox
```java
sessionManager.publishMessage(sessionId, userMessage, EventType.MESSAGE, MessageChannel.SYSTEM);
sessionManager.publishMessage(sessionId, journalMessage, EventType.JOURNAL, MessageChannel.JOURNAL);
```

### 订阅 Inbox 事件
```java
session.getEventBus().inBoxSubscribe()
...
### 发布消息到 Outbox
```java
session.getEventBus().outBoxPublish(responseMessage);
```

### 订阅 Outbox 消息
```java
session.getEventBus().outBoxSubscribe()
...
## 注意事项
1. EventBus 是纯工具类（非 Spring Bean），使用静态方法，无需依赖注入。推荐优先使用 SessionManager 的路由方法
...
8. EventType 默认值为 MESSAGE，MessageChannel 默认值为 USER，现有 publishMessage 调用完全向后兼容
9. `publishMessage(sessionId, message, eventType)` 会自动通过 `MessageChannel.fromEventType()` 推导通道，无需手动指定
10. 需要覆盖默认映射时（如心跳使用 SYSTEM 通道而非 USER），使用四参数重载 `publishMessage(sessionId, message, eventType, messageChannel)`
