...
public class EvolveCycleTool extends AbstractTool<EvolveCycleTool.Input> {
    ...
    private EvolveCycleTool(EvolutionEngine evolutionEngine, FileSystemSessionManager fileSystemSessionManager) {
        super("evolve_run_cycle", DESCRIPTION, Input.class);
        Assert.notNull(evolutionEngine, "evolutionEngine不能为null");
        Assert.notNull(fileSystemSessionManager, "sessionManager不能为null");
    ...
    private List<String> extractConversationTexts(String sessionId) {
        try {
            var session = fileSystemSessionManager.getById(sessionId);
            if (session == null) {
                return List.of();
            }
    ...
    public static class Builder {
        ...
        private FileSystemSessionManager fileSystemSessionManager;
        ...
        public Builder sessionManager(FileSystemSessionManager fileSystemSessionManager) {
            this.fileSystemSessionManager = fileSystemSessionManager;
            return this;
        }

        public EvolveCycleTool build() {
            return new EvolveCycleTool(evolutionEngine, fileSystemSessionManager);
        }
    }
}
