...
public final class TurnWindowCompactionStrategy implements CompactionStrategy {
	...
	public CompactionResult compact(CompactionRequest request) {
		...
		List<MessageEvent> events = request.events();
		...
		List<MessageEvent> preamble = new ArrayList<>();
		int firstUserIdx = 0;
		...
		List<MessageEvent> afterPreamble = real.subList(firstUserIdx, real.size());

		// 3. Group into turns — each turn starts at a root-level user message
		List<List<MessageEvent>> turns = groupIntoTurns(afterPreamble);
		...
		List<List<MessageEvent>> archivedTurns = turns.subList(0, toArchiveCount);
		List<List<MessageEvent>> keptTurns = turns.subList(toArchiveCount, turns.size());

		List<MessageEvent> archived = archivedTurns.stream().flatMap(List::stream).toList();
		List<MessageEvent> kept = keptTurns.stream().flatMap(List::stream).toList();
	 ...
	 */
	private static List<List<MessageEvent>> groupIntoTurns(List<MessageEvent> events) {
		List<List<MessageEvent>> turns = new ArrayList<>();
