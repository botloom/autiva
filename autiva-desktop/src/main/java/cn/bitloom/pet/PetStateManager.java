...
public class PetStateManager {
    ...
    public PetStateManager(FileSystemSessionManager fileSystemSessionManager) {
    ...
    private void analyzeAndSetPetType() {
        try {
            // 获取所有用户会话的消息
            var allSessions = fileSystemSessionManager.getAllUserSessions();
    ...
    private void loadState() {
        try {
            ...
            if (Files.exists(path)) {
                ...
                state = JsonUtils.fromJson(json, PetState.class);
    ...
    private void saveState() {
        try {
            ...
            String json = JsonUtils.toPrettyJson(state);
