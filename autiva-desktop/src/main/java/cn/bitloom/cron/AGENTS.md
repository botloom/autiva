...
- `SessionManager`: 会话管理器（@Lazy，避免与 FileSystemSessionManager → Toolkit → CronManager 的循环依赖）
...
**消息发布方式：**
- 用户会话心跳：通过 `session.publish(MessageEvent.userMessage(sessionId, text))` 发送
- EVOLVER 心跳：通过 `session.publish(MessageEvent.userMessage(sessionId, text))` 发送
...
**消息类型：**
- `MessageEvent.userMessage(sessionId, text)`: 构建用户消息事件

**发送方式：**
```java
session.publish(MessageEvent.userMessage(sessionId, text));
```
...
8. 消息发布时使用 `session.publish(MessageEvent.userMessage(sessionId, text))` 构建并发布消息
