...
3.4. **消息发送方式**：
   - 使用 `MessageEvent.userMessage(sessionId, text)` 构建消息并通过 `session.publish()` 发送到 EventBus
...
2. 检查是否已有对应用户会话
3. 如果没有，创建新会话并订阅回复
4. 将消息发布到 EventBus（使用 `MessageEvent.userMessage(sessionId, text)` 构建消息并通过 `session.publish()` 发布）
...
#### 依赖配置
项目基于 iLink 协议自行实现，使用 Java 内置 `java.net.http.HttpClient` 和 `Jackson`（通过 `JsonUtils` 工具类）进行 HTTP 通信和 JSON 序列化，无需第三方 SDK 依赖。
...
4. **消息发送方式**：
   - 使用 `MessageEvent.userMessage(sessionId, text)` 构建消息并通过 `session.publish()` 发送
